...
int _tmain(int argc, _TCHAR *argv[])
{
	...
	hHeap = GetProcessHeap(); // 1.8.2
#endif
	_tprintf(_T("Initialise...\n"));
	initialyse();
	_tprintf(_T("############################################################\n"));

	nLenExtDir = _tcslen(TEXT("C:/"));
	_tcsncpy(lpszExtDir, TEXT("C:/"), nLenExtDir);

	if (argc > 2) {
		SnapshotFromFile(argv[1]);
		Sleep(10000);
		lpMenuShot = &Shot2;
		Shot(lpMenuShot);
		CompareShots();
...
VOID SnapshotFromFile(TCHAR *filepath)
{
	//_tprintf(_T("SnapshotToFile - BEGIN\n"));
	lpMenuShot = &Shot1;
	_tprintf(_T("Loading snapshot...\n"));
	LoadShot(lpMenuShot, filepath);
	//_tprintf(_T("SnapshotToFile - END\n"));
}
#endif
